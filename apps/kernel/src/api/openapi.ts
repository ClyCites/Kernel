    ...
    paths: {
      ...
      '/media': {
        options: {
          ...
          summary: 'What this kernel will accept',
          description:
            'The tus discovery request. `Clycites-Accept-Types` and `Clycites-Max-Chunk` are ' +
            'extensions: a client on a metered link needs to know the chunk ceiling before it ' +
            'starts, and needs to know the type is acceptable before it spends the bytes.',
          responses: {
            '204': {
              ...
              headers: {
                ...
                'Clycites-Accept-Types': { schema: { type: 'string' } },
                'Clycites-Max-Chunk': { schema: { type: 'integer' } },
              },
            },
        ...
        post: {
          ...
          description:
            'Declares the SHA-256, the type and the length before any byte is sent, so a file that ' +
            'will be refused is refused before it is paid for. The declared hash is of the file as ' +
            'it leaves the device; the hash the kernel stores and anchors is computed after metadata ' +
            'is stripped, and the two differ whenever the file carried any. Both are returned when ' +
          ...
          parameters: [
            ...
            },
            {
              ...
              required: true,
              description:
                'tus metadata: comma-separated `key base64(value)` pairs. `hash` and `mime` are required.',
              schema: { type: 'string' },
            },
          ],
          responses: {
            ...
            '400': problemResponse(
              'The declared hash, type or size is not acceptable. `reason` is one of ' +
                '`hash_malformed`, `type_refused`, `too_large`.',
            ),
            '404': problemResponse('No verified subject; an upload must have an owner.'),
            '503': problemResponse('This kernel has no object store configured.'),
          },
        },
      },
      '/media/{id}': {
        head: {
          tags: ['media'],
          operationId: 'uploadOffset',
          summary: 'How much of my upload arrived',
          description:
            'The resume point. A client that lost the network mid-file asks this and continues from ' +
            'the returned offset. Only the party that opened the session may ask; anyone else gets 404.',
          parameters: [
            {
              name: 'id',
              in: 'path',
          ...
          responses: {
            '200': {
              ...
              headers: {
                ...
                'Clycites-Upload-State': {
                  schema: { type: 'string', enum: ['open', 'complete', 'rejected'] },
                },
                'Clycites-Upload-Rejection': { schema: { type: 'string' } },
                'Clycites-Content-Hash': { schema: { type: 'string' } },
              },
            },
            '404': problemResponse('No such session, or not yours.'),
          },
        },
        patch: {
          tags: ['media'],
          operationId: 'appendChunk',
          summary: 'Send the next piece',
          description:
            'A chunk at any offset other than the current one is a conflict, not a correction — ' +
            'accepting it would let a retry that crossed with a success silently duplicate bytes. ' +
            'On the last chunk the kernel verifies the hash, checks the bytes really are the type ' +
            'declared, strips metadata and stores the result; the response then carries the content ' +
            'hash to cite.',
          parameters: [
            {
              name: 'id',
          ...
          responses: {
            '204': {
              ...
              headers: {
                'Upload-Offset': { schema: { type: 'integer' } },
                'Clycites-Upload-State': { schema: { type: 'string' } },
                'Clycites-Content-Hash': {
                  schema: { type: 'string' },
                  description: 'Present once the object is stored. This is what a record cites.',
                },
                'Clycites-Metadata-Stripped': { schema: { type: 'string' } },
              },
            },
            '400': problemResponse('Wrong content type, or a missing offset.'),
            '409': problemResponse('The offset does not match; re-read it with HEAD.'),
            '404': problemResponse('No such session, or not yours.'),
          },
        },
      },
      '/media/{hash}/url': {
        get: {
          ...
          summary: 'A short-lived link to the bytes',
          description:
            'The bucket is not public and no object is reachable without passing through here. ' +
            'The answer is decided by the records that cite the object: a caller who may read at ' +
            'least one of them gets a signed URL valid for minutes, and everyone else gets 404 — ' +
            'not 403, which would confirm the photograph exists. The disclosure is logged, so it ' +
            'appears in the subject\u2019s own disclosure list alongside record reads.',
          parameters: [
            {
              name: 'hash',
              in: 'path',
